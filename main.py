...
class App:
    ...
    def __init__(self):
        ...
        self.sim_runner.post_physics_step_hook = self._render_audio_after_physics_step
    ...
    def _render_audio_after_physics_step(self, ui_state, step_index):
        ...
        # per-frame total stays exact.
        blocks = cap.blocks_owed(1.0 / speedmult)
        if blocks <= 0:
            return

        streamline = ui_state.streamline
        seed = (streamline.pinned_seed if streamline.seed_pinned
                else self._cursor_seed(ui_state))
        for block in self.streamline_service.render_audio_blocks(
                self.sim.can, seed, streamline,
                self.audio_service, ui_state.audio, blocks,
                prev_texture=self._prev_canvas(),
                samples_per_physics_step=self._samples_per_physics_step(ui_state)):
            cap.add_block(block)

    def _cursor_seed(self, ui_state):
        """Seed position in the simulation's aspect-corrected space."""
        tex_x, tex_y = self.camera.screen_to_tex(
            ui_state.mouse_pos, tex_size=self.sim.can.size
        )
        cw, ch = self.sim.can.size
        ca = cw / ch
        return ((tex_x * 2.0 - 1.0) * ca ** 0.5,
                (tex_y * 2.0 - 1.0) / ca ** 0.5)
