...
class App:
    ...
    def __init__(self):
        ...
        # Per-frame tracer scheduling, resolved in _prepare_streamline_frame()
        # and consumed by the post-physics-step hook.
        self._streamline_frame_dt = 0.0
        self._streamline_frame_samples = 0
        self._streamline_stepped_this_frame = False
        ...
        # Steps the tracer between physics steps so it sees every canvas
    ...
    def orchestrate_frame(self):
        ...
        # 5.9. Streamline tracer clock and audio stream lifecycle.
        #
        # This has to settle BEFORE the physics runs, because the tracer is now
        # stepped from inside the physics loop (post_physics_step_hook) so that
        # it sees every canvas frame rather than only the last of each
        # speedmult batch. The hook needs this frame's dt and a live stream.
        self._prepare_streamline_frame(ui_state)
        ...
        # 7.6. Finish the tracer's frame and draw its overlay.
        ...
        if (ui_state.streamline.enabled or ui_state.audio.enabled
            ...
            seed = self._streamline_seed(ui_state)

            if self.audio_capture is not None:
                # Offline: audio is generated by the per-physics-step hook so
                # it samples an advancing canvas. Those same dispatches
                # advance the tracer, so the wall-clock path must not also
                # run or the particles would double-step.
                pass
            elif not self._streamline_stepped_this_frame:
                # The physics loop did not run (paused, or the sim is not
            ...
            if self.audio_capture is None and self.audio_service.active:
    ...
    def _prepare_streamline_frame(self, ui_state):
        ...
        The tracer is stepped from inside the physics loop now, so everything
        the hook depends on - this frame's dt, a live audio stream, the locked
        dispatch schedule, the seed pin - has to be resolved before step 6.
        """
        streamline = ui_state.streamline
        audio = ui_state.audio

        # The tracer runs on its own clock, so it keeps its rate whether or not
        # the physics is paused and regardless of the frame rate.
        now = time.time()
        ...
        active = (streamline.enabled or audio.enabled
                  or self.audio_capture is not None)
        # Banking time while the overlay is off would fire a catch-up burst the
        # moment it is switched back on.
        self._streamline_frame_dt = dt if active else 0.0
        self._streamline_frame_samples = 0
        self._streamline_stepped_this_frame = False
        if not active:
            return
        ...
        if ui_state.middle_click_this_frame:
            ...
            if streamline.seed_pinned:
                streamline.pinned_seed = self._cursor_seed_world(ui_state)

        # An offline capture owns the tracer for the duration, so live playback
        # stays down even though audio.enabled is still set - it records the
        # user's intent to resume once the render finishes.
        ...
        if want_live and self.audio_service.active:
            # Locked to the audio contract: one block of samples per dispatch,
            # at the device's rate. Stash the user's own values first and
            # restore them on release, so the locked numbers are never what
            # gets persisted or left behind.
    ...
    def _cursor_seed_world(self, ui_state):
        """Cursor position in the simulation's world space.

        Screen pixels -> texture [0,1] -> world. Streamers live in the
        simulation's aspect-corrected space (+/- sqrt(ca), 1/sqrt(ca)), not a
        [-1,1] square, so they read the same sensor geometry real particles do.
        """
        tex_x, tex_y = self.camera.screen_to_tex(
            ui_state.mouse_pos, tex_size=self.sim.can.size
        )
        cw, ch = self.sim.can.size
        ca = cw / ch
        hx, hy = ca ** 0.5, 1.0 / ca ** 0.5
        return ((tex_x * 2.0 - 1.0) * hx, (tex_y * 2.0 - 1.0) * hy)

    def _streamline_seed(self, ui_state):
        """The seed the tracer should use: pinned point, or the cursor."""
        streamline = ui_state.streamline
        if streamline.seed_pinned:
    ...
    def _step_streamlines_with_physics(self, ui_state, step_index, total_steps):
        ...
        Runs from SimulationRunner.post_physics_step_hook. This is what lets
        the field interpolation work at speedmult > 1: the canvas ping-pongs,
        so only here do `canvas` and `canvas_prev` describe the interval the
        tracer is stepping through. Called after the whole batch instead, the
        tracer would interpolate one interval and jump over the other
        speedmult-1 - the residual seam then sits at the render rate rather
        than the physics rate, which is the "zippery" artifact.

        While recording, the offline capture is driven from the same place and
        for the same reason - it owns the tracer for the duration, so it runs
        instead of the realtime path, never alongside it.
        """
        if self.audio_capture is not None:
            self._render_audio_after_physics_step(ui_state, step_index,
                                                  total_steps)
            return
        streamline = ui_state.streamline
        audio = ui_state.audio
        if not (streamline.enabled or audio.enabled):
            return
        if not streamline.running:
            return

        audio_live = audio.enabled and self.audio_service.active
        if audio_live:
            # The sound device sets the pace. Ask once per render frame -
            # blocks_wanted() drains an accumulated debt, so calling it per
            # physics step would hand out the same time repeatedly - then
            # spread that budget evenly across the frame's physics steps so
            # each interval carries its own share of the samples.
            if step_index == 0:
                self._streamline_frame_samples = (
                    self.audio_service.blocks_wanted(self._streamline_frame_dt)
                    * AUDIO_BLOCK)
            owed = self._streamline_frame_samples / max(1, total_steps)
        else:
            # Visual only: the tracer's own schedule, divided across the
            # physics steps of this render frame.
            rate = max(1.0, float(streamline.dispatch_hz)
                       * max(1, int(streamline.steps_per_dispatch)))
    ...
    def _render_audio_after_physics_step(self, ui_state, step_index,
                                         total_steps):
        """Generate the audio one physics step is worth, in lockstep with it.

        Called after each physics step while recording. Generating a whole
        video frame's audio in one go instead meant all ~800 samples sampled
        the same frozen canvas and the field then jumped speedmult steps at
        once - a staircase at the video frame rate, audible as chunkiness.
        Interleaving keeps the field advancing underneath the samples, the
        way it does in realtime.
        """
